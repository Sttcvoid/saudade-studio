...
try {
    fetch('https://api.eightyseven.ru/saudade/members').then(function (response) {
        ...
        json.categories.forEach(category => {
            html += `<h1 class="text-4xl font-bold font-manrope mt-6" style="font-family:var(--main-font-bold)">${category.name}</h1>`;

            category.subcategories.forEach(subcategory => {
                html += `<div class="text-2xl font-bold mt-3 mb-8 font-manrope">${subcategory.name}</div>`;
                ...
                subcategory.members.forEach(member => {
                    ...
                    member.contacts.forEach(contact => {
                        var contactIcon;
                        if (contact.id == "vk") contactIcon = `<svg fill="#ffffff" viewBox="0 0 42 26" class="${contacts == `` ? `mr-2` : ``} h-5 w-5" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                        ...
                        else if (contact.id == "discord") contactIcon = `<svg fill="#ffffff" viewBox="0 0 24 24" class="${contacts == `` ? `mr-2` : ``} h-5 w-5" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                        ...
                        else if (contact.id == "tg") contactIcon = `<svg fill="#ffffff" viewBox="0 0 24 24" class="${contacts == `` ? `mr-2` : ``} h-5 w-5" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                        ...
                        else if (contact.id == "mail") contactIcon = `<svg stroke="currentColor" fill="none" stroke-width="0" viewBox="0 0 24 24" class="${contacts == `` ? `mr-2` : ``} h-5 w-5" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                        ...
                        contacts += `<a href="${contact.url}" class="inline-flex cursor-pointer mt-2 border-0 px-5 py-2.5 font-medium leading-5 text-zinc-50 transition ease-in-out bg-zinc-800 rounded-lg focus:outline-none focus:ring-0 hover:bg-zinc-700 active:bg-zinc-600 select-none">
                                ${contactIcon}${contacts == `` ? `Связаться` : ``}
                            </a>`
                    });
